...
window.THEME_PREFIX = "cmls_base"; // Polyfill for CSS Vars

window.self.document.addEventListener("DOMContentLoaded", function () {
  if (window.self.document.body.classList.contains('logged-in')) {
    console.log('Note: child theme CSS links should be added to the array window.additionalCssIncludes so vars may be ponyfilled!');
  }
});

function setupCssVarsPonyfill() {
  var cssIncludes = ["link#" + window.THEME_PREFIX + "_customizer_vars-css", "style#" + window.THEME_PREFIX + "_customizer_vars-inline-css", "link#" + window.THEME_PREFIX + "_style-css"];
...
if (!window.self.CSS || !window.self.CSS.supports('color', 'var(--fake-var)')) {
  window.self.document.addEventListener("DOMContentLoaded", function () {
    ...
    window.self.document.getElementsByTagName('head')[0].appendChild(cssScr);
  });
}
